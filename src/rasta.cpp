...
{
	OnOffMap onoff;
	memset(onoff.on_off, true, sizeof(onoff.on_off));
	...
	while (getline(f, line))
	{
		...
		{
			onoff.on_off[l][target] = (value == "ON");
		}
		++y;
	}

	// Set OnOff map in evaluation context
	m_eval_gstate.m_onoff = &onoff;
}
...
bool RastaConverter::ProcessInit()
{
	...
	m_eval_gstate.m_width = m_width;
	m_eval_gstate.m_height = m_height;
	m_eval_gstate.m_picture = m_picture.data();
	m_eval_gstate.m_picture_all_errors = m_picture_all_errors_array;
	m_eval_gstate.m_max_evals = cfg.max_evals;
	m_eval_gstate.m_save_period = cfg.save_period;
	m_eval_gstate.m_cache_size = cfg.cache_size;
	...
	m_executors.resize(cfg.threads);
	...
	for (size_t i = 0; i < m_executors.size(); ++i)
	{
		...
		m_executors[i]->Init(m_width, m_height, m_picture_all_errors_array,
			m_picture.data(), m_eval_gstate.m_onoff, &m_eval_gstate,
			solutions, randseed, cfg.cache_size, i);
...
void RastaConverter::MainLoop()
{
	...
	// spin up only one executor -- we need its result before the rest can go
