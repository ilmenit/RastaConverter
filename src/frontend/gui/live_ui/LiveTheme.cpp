...
ImU32 kBadgeDual   = IM_COL32(0xA8, 0x3D, 0x57, 0xFF);
...
void ApplySemanticPalette(UiTheme selected)
{
	...
	if (selected == UiTheme::Light) {
		...
		kBadgeDual   = IM_COL32(0x95, 0x28, 0x43, 0xFF);
		return;
	}
	if (selected == UiTheme::HighContrast) {
		...
		kBadgeDual   = IM_COL32(0xFF, 0x9A, 0xAE, 0xFF);
		return;
	}
	...
	kBadgeDual   = IM_COL32(0xA8, 0x3D, 0x57, 0xFF);
}
